...
const CarNotification = ({ notifications, rows, setIsModalVisible }) => {
  ...
  useEffect(() => {
    if (notifications.length > 0){
      setIsModalVisible(true); // Modal is visible initially true
      ...
      // Hide modal when there is no update
      setIsModalVisible(false);
      setIsVisible(false);
    }
    },[notifications, setIsModalVisible]);
      ...
      {isVisible && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white border-4 border-red-500 p-6 rounded shadow-md max-w-md w-full">
            ...
            <h2 className="text-xl font-bold">{t("upcomingUpdates")} <FontAwesomeIcon icon={faBell} className='pl-2 shadow-2xl animate-ping'/></h2>
              ...
              {notifications.map((notification, index) => (
                <li key={index} className="mb-2">
                  ...
                  >
                    ...
                    {`${index+1}.${t("Car")}{notification.carID}: {notification.date}`}
                  </button>
                </li>
              ))}
            </ul>
